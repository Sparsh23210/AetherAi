...
export default function SubmitToolPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mathematically define initial popularity based on quality and speed, with a slight random boost for new launches
      const baseScore = (formData.speedScore * 0.4) + (formData.qualityScore * 0.6);
      const randomBoost = Math.random() * 1.5; // Random boost between 0 and 1.5
      const initialPopularity = Math.min(10, Math.round(baseScore + randomBoost));
      ...
      // 3. Link to Category Table (for global filtering)
      if (toolData) {
        ...
        if (catData) {
          try {
            await fetch(`http://localhost:5000/api/tools/${toolData.id}/categories`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ categoryId: catData.id })
            });
          } catch (linkErr) {
            console.error('[Launch] Category linking failed:', linkErr);
          }
        }
      }

      console.log('[Launch] SUCCESS! Startup live.');
      clearTimeout(timeoutId);
      setSuccess(true);
      setTimeout(() => router.push('/founders'), 1500);
    } catch (err: any) {
  ...
  if (success) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-center p-4">
        ...
        <h1 className="text-4xl font-black mb-4 text-white">Startup Launched!</h1>
        <p className="text-slate-400 text-lg mb-8">Your tool is now live on the launchpad and directory.</p>
    ...
    <AuthGate>
      ...
      <main className="max-w-3xl mx-auto px-4 py-20">
        <div className="mb-16 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-black mb-4 tracking-tight">Launch Your Startup</h1>
          <p className="text-slate-400 text-lg">Reach thousands of AI enthusiasts by listing your product.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-slate-900/30 border border-slate-800 rounded-[40px] p-8 md:p-12 backdrop-blur-xl">
          ...
          <div className="pt-8 flex items-center justify-between border-t border-slate-800">
            <Link href="/founders" className="text-slate-500 hover:text-white transition font-medium">Cancel</Link>
            <button type="submit" disabled={loading} className="bg-indigo-600 hover:bg-indigo-500 text-white px-10 py-4 rounded-2xl font-bold transition flex items-center gap-3 shadow-xl shadow-indigo-600/20 disabled:opacity-50 active:scale-95">
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Launch Startup <ArrowRight size={20} /></>}
            </button>
          </div>
        </form>
      </main>
